...
class HomeController extends Controller
{
    public function index()
    {
        ...
        // Hitung total amount untuk Document PR/PO
        $documentPRPOAmount = DB::table('hpp1')
            ->whereIn('notification_number', $lhppNotificationNumbers) // Sudah ada di LHPP
            ->sum('total_amount'); // Ambil total_amount dari HPP
    
        // Jika Document PR/PO sudah terpenuhi, nolkan ketiga amount lainnya
        if ($documentPRPOAmount > 0) {
            $documentOnProcessHPPAmount = 0;
            $approvalProcessHPPAmount = 0;
            $documentOnProcessPOAmount = 0;
        }
        ...
        $totalAmount2 = $documentPRPOAmount;
    ...
    public function realisasiBiaya(Request $request)
    {
        try {
            ...
            // Ambil data notification_number dari tabel LPJ berdasarkan tahun
            $lpjs = DB::table('lpjs')
            ...
            // Hitung total realisasi biaya berdasarkan notification_number dari tabel LPJ
            $realisasiBiaya = DB::table('hpp1')
                ->join('lpjs', 'hpp1.notification_number', '=', 'lpjs.notification_number')
                ->whereIn('hpp1.notification_number', $lpjs)
                ->whereBetween(DB::raw('YEAR(lpjs.update_date)'), [$startYear, $endYear])
                ->select(
                    DB::raw('YEAR(lpjs.update_date) as year'),
                    DB::raw('SUM(hpp1.total_amount) as total')
                )
                ->groupBy(DB::raw('YEAR(lpjs.update_date)'))
                ->get();
