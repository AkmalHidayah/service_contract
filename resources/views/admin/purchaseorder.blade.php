        ...
        <div class="bg-white border border-gray-300 rounded-lg shadow-md overflow-x-auto">
            <table class="min-w-full text-xs text-left text-gray-500">
                <thead class="bg-gray-200 text-gray-700 uppercase font-medium">
                    <tr>
                        <th class="px-4 py-2">Nomor Notifikasi</th>
                ...
                <tbody>
                    @foreach($notifications as $notification)
                        <form action="{{ route('purchaseorder.update', $notification->notification_number) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex flex-col items-start space-y-1">
                                        ...
                                        <input id="po_document_{{ $notification->notification_number }}" type="file" name="po_document" 
                                            class="hidden">
                                        @if($notification->purchaseOrder && $notification->purchaseOrder->po_document_path)
                                            <span class="text-xs text-gray-600 mt-1">{{ basename($notification->purchaseOrder->po_document_path) }}</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-2">
