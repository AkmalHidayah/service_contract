                ...
                <table class="min-w-full bg-white text-sm">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-2 border-b text-left text-xs sm:text-sm text-blue-500">Nomor Notifikasi</th>
                    ...
                    <tbody>
                        @foreach ($lhppDocuments as $lhpp)
                            <tr>
                                ...
                                <a href="{{ route('approval.lhpp.show', ['notification_number' => $lhpp->notification_number]) }}" class="bg-blue-500 text-white px-2 py-1 rounded-lg hover:bg-blue-700 text-xs flex items-center justify-center" target="_blank">
                                ...
                                <!-- Logika untuk aksi tanda tangan -->
                                    @if(auth()->user()->jabatan == 'Manager')
                                        @if(is_null($lhpp->manager_signature) && auth()->user()->unit_work == 'Workshop & Construction')
                                            <!-- Manager Workshop -->
                                            <button type="button" class="bg-blue-500 text-white px-2 py-1 rounded text-xs" onclick="openSignPad('{{ $lhpp->notification_number }}', 'manager')">Tanda Tangan Manager Workshop</button>
                                            <button type="button" class="bg-red-500 text-white px-2 py-1 rounded text-xs" onclick="rejectDocument('{{ $lhpp->notification_number }}', 'manager')">Reject</button>
                                        @elseif(!is_null($lhpp->manager_signature) && is_null($lhpp->manager_signature_requesting) && auth()->user()->unit_work == $lhpp->unit_kerja)
                                            <!-- Manager Unit Peminta -->
                                            <button type="button" class="bg-blue-500 text-white px-2 py-1 rounded text-xs" onclick="openSignPad('{{ $lhpp->notification_number }}', 'manager_requesting')">Tanda Tangan Manager Peminta</button>
                                            <button type="button" class="bg-red-500 text-white px-2 py-1 rounded text-xs" onclick="rejectDocument('{{ $lhpp->notification_number }}', 'manager_requesting')">Reject</button>
                                        @elseif(!is_null($lhpp->manager_signature_requesting) && is_null($lhpp->manager_pkm_signature) && auth()->user()->unit_work == $lhpp->kontrak_pkm)
                                            <!-- Manager PKM -->
                                            <button type="button" class="bg-green-500 text-white px-2 py-1 rounded text-xs" onclick="openSignPad('{{ $lhpp->notification_number }}', 'manager_pkm')">Tanda Tangan Manager PKM</button>
                                            <button type="button" class="bg-red-500 text-white px-2 py-1 rounded text-xs" onclick="rejectDocument('{{ $lhpp->notification_number }}', 'manager_pkm')">Reject</button>
                                        @else
                                            <!-- Tidak ada aksi untuk kondisi ini -->
                                            <span class="text-gray-500 text-xs">Tidak Ada Aksi</span>
                                        @endif
                                    @endif

                                    <!-- Jika semua tanda tangan sudah selesai -->
                                    @if(!is_null($lhpp->manager_signature) && !is_null($lhpp->manager_signature_requesting) && !is_null($lhpp->manager_pkm_signature))
                                        <span class="text-green-500 text-xs">Sudah Ditandatangani</span>
                                    @endif


                                    <!-- Form untuk hasil Quality Control -->
                                    @if(auth()->user()->jabatan == 'Manager' && auth()->user()->unit_work == 'Workshop & Construction')
